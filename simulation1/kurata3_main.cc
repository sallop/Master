...
// [kurata's matrix _denergy1, _energy4] : [date's matrix _denergy0]
  ...
  for(size_t t=1; t<T; t++){
    ...
    gibbs_P = zerov(0x01<<D);	// plot a gibbs sampling statistics
    vf = zerov(D);		// fantasy particle
    for(size_t m=0; m<M; m++){
      //kstep_gibbs(1,vf,L,alpha);
      kstep_gibbs_count_onbit(K, vf, L, temperature, gibbs_P);
      model += outer_prod(vf,vf);// mean value
    }
    model /= M;
    gibbs_P /= K*M;
