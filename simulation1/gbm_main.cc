...
int main(int argc, char **argv)
{
  ...
  char prefix[32]="g1", odir[32] = "dir-dat";
...
// #define TV(v,i,size) v[(i)%(size)]
// #define TM(m,i,j,rowsz,colsz) m(i%rowsz,j%colsz)
...
// 	      << it->first << "," 
// 	      << "("
// 	      << it->second.first // registery number
// 	      << ","
// 	      << it->second.second // Pattern
// 	      << ")"
// 	      << std::endl;
//   for (Patterns::iterator it=all_v.begin();
//        it != all_v.end();
//        it++){
//     int key=btod(*it);
//     std::cout << key		// dtob(pattern)
// 	      << "("
// 	      << dtob_tbl[key].first // registery number
// 	      << ","
// 	      << dtob_tbl[key].second // registery pattern
// 	      << ")"
// 	      << std::endl;
//   }
//#define SYM(m,i,j) m(i,j) = m(j,i)
  //1. Randomly initialize mu and
  F = init_layer(D+P,connection_scale);
//   UDITER(L){ SYM(L,i,j) = 1.0*(drand48()-0.5);}
//   UDITER(J){ SYM(J,i,j) = 1.0*(drand48()-0.5);}
//   EACHM(W){ W(i,j) = 1.0*(drand48()-0.5);}
