...
public:
  ...
  virtual double PushUp   (int j, Pattern& v, Pattern& h)     = 0;
...
public:
  ...
  double PushUp(int j, Pattern& v, Pattern& h)
  {
    size_t i,m;
    double term1=0.0, term2=0.0;
    for(i=0; i<D_; i++) term1 += W_(i,j)*v(i);
  ...
  void Learn()
  {
    ...
    Pattern    a_data(D_,0), a_model(D_,0);
    Pattern    b_data(D_,0), b_model(D_,0);
    ZeroLayer L_zero(D_,D_), W_zero (D_,P_), J_zero (P_,P_);
    ZeroPattern  v_zero(D_), h_zero(P_);
    ZeroPattern  a_zero(D_), b_zero(P_);
	...
	for(k=0; k<K_; k++){
	  j=rand()%P_;
	  i=rand()%D_;
	  hf[j] = (PushUp  (j,vf,hf) > drand48()) ? ONBIT : OFFBIT;
	...
	b_ = b_ + alpha_*(b_data/N_ - b_model/M_);
	symmetrization_U<double>(L_);
	symmetrization_U<double>(J_);
      }
      alpha_ = 1.0/(t + 1);
...
int main(int argc, char **argv)
{
  ...
  BM *gbm1 = new GBMBiasIterate  (D, P, K, M, N, T, alpha, prob_Q, all_v, all_h);
  gbm1->Learn();
  return 0;
}
