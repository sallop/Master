...
int main(int argc, char **argv)
{
  ...
  int result;
  while ((result = getopt(argc,argv, "D:P:K:M:N:T:d:p:t")) != -1){
    switch (result){
  ...
  char base[32]; sprintf(base,"%s/%s%dx%d",odir,prefix,D,D);
  ...
  for (idx=&ftbl[0]; idx->identify != NULL; idx++){
    std::string fname = std::string(base) + std::string(idx->identify);
    idx->os->open(fname.c_str());
  }
